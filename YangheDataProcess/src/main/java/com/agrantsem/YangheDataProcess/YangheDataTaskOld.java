...
public class YangheDataTaskOld extends TimerTask {
    ...
    public void dayWork(String dateFormate, boolean sendEmail) {
        ...
        String ttVrdOrginPathName = (YangPath + TTVrdName).replaceAll("yymmdd", dateFormate);
        String ttVrdRetPathName = (YangPath + prex + TTVrdName).replaceAll("yymmdd", dateFormate);
        String ttVrdDone=ttVrdRetPathName+ ".done";

        //从ftp获取百分点反馈的
        String BFDVrdRetPathName = (YangPath + BFDVrdName).replaceAll("yymmdd", dateFormate);
        String BFDVrdRetDone = BFDVrdRetPathName+".done";

        if (TxtFileUtil.isExists(showDone) && TxtFileUtil.isExists(ttReqDone) && TxtFileUtil.isExists(ttVrdDone)&&TxtFileUtil.isExists(BFDVrdRetDone) ) {
        ...
        if (isHdfsDone(dateFormate)) {
            ...
            downShowTTResultFromHdfs(dateFormate, showOrginPathName, ttReqOrginPathName, ttVrdOrginPathName,BFDVrdRetPathName);
            if (TxtFileUtil.isExists(showOrginPathName) && TxtFileUtil.isExists(ttReqOrginPathName)&&TxtFileUtil.isExists(BFDVrdRetPathName)) {
                boolean ret = dealShowTT(showOrginPathName, showRetPathName,
                        ttReqOrginPathName, ttReqRetPathName, showDone, ttReqDone,BFDVrdRetPathName,BFDVrdRetDone);
                ...
                if (ret && sendEmail) {
                    ...
                    ret = sender.sendHTMLOut(mail_targets,
                            ...
                            new String[]{showRetPathName,ttReqRetPathName,ttVrdRetPathName});
                    if (ret == true) {
                ...
                LOGGER.warn(showOrginPathName + " or " + ttReqOrginPathName + "or" + ttVrdOrginPathName
    ...
    private void downShowTTResultFromHdfs(String currentDate, String showOrginPathName, String ttReqOrginPathName, String ttVrdOrginPathName,String BFDVrdRetPathName) {
        ...
        if (!TxtFileUtil.isExists(ttVrdOrginPathName)) {
            String comm_getTTVrd = comm_getTTVrdPrex + ttVrdOrginPathName;
            comm_getTTVrd = comm_getTTVrd.replaceAll("yymmdd", currentDate);
            Worker.exec(comm_getTTVrd, 5 * 60 * 1000);
        }

        if (!TxtFileUtil.isExists(BFDVrdRetPathName)){
            String comm_bfdvrd = comm_bfdbrdPrex + BFDVrdRetPathName;
            comm_bfdvrd = comm_bfdvrd.replaceAll("yymmdd", currentDate);
            Worker.exec(comm_bfdvrd, 5 * 60 * 1000);
            String BFDVrdRetDone = BFDVrdRetPathName+".done";
            TxtFileUtil.saveDone(BFDVrdRetDone);
        }
    }
    ...
    public boolean dealShowTT(String showOrginPathName, String showRetPathName,
                              ...
                              String ttDone,String BFDVrdRetPathName,String BFDVrdRetDone) {
        ...
        if (!TxtFileUtil.isExists(ttDone)&&TxtFileUtil.isExists(BFDVrdRetDone)) {
            TxtFileUtil.deleteFile(ttRetPathName);
            HashSet<String> yangheset = TxtFileUtil.getYangheSetFromFile(YangheMagicPathName);
            //boolean ret = TxtFileUtil.processTTFile(ttOrginPathName, ttRetPathName, yangheset, YangheMagicPathName,BFDVrdRetPathName);
    ...
    public static void main(String[] args) {
        YangheDataTaskOld task = new YangheDataTaskOld();
