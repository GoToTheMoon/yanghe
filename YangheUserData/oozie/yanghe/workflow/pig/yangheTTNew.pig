...
dspttYHAll = filter dspttAll  by cid == $selectCid and  channel in ('YOUKU','QAX'); --yangheCid ;--只要洋河的点击数据
...
contentCategory = load '/user/le.xiang/yanghe/data/contentCategory.txt' USING PigStorage()  as (agContctgr: chararray,channel: chararray,channelContctgr: chararray);-- using parquet.pig.ParquetLoader();
...
------------------------------showd的cm结果（洋河id，agsid，qaxid，优酷id,reqid）---------
cmDup = load '$inputDirDspShowCmIp' USING PigStorage()  as (yangheid: chararray,agsid: chararray,qaxid: chararray,youkuid: chararray,reqid: chararray);
--cmFilter = filter cmDup yangheid is not null and yangheid!='';--20170224
cm= DISTINCT cmDup;--20170224改成cmFilter
userCmDuplicateAgsid = join cm by agsid LEFT OUTER,user by agsid ;--拿agsid去找标签
userCm = foreach userCmDuplicateAgsid GENERATE cm::reqid as reqid, cm::yangheid as yangheid,cm::agsid as agsid,cm::qaxid as qaxid,cm::youkuid as youkuid,user::tag as tag;--一个channelid 可能对应多个agsid，tag
...
orderyanghedata = order yanghedata by yangheid;
STORE orderyanghedata INTO '$outputdirIp' USING PigStorage('\t');

--0 16 * * * hadoop fs -get /user/le.xiang/adview/ttimpid-`date  +"%Y%m%d" -d  "-1 days"`/part-00000 ttimpid-`date  +"%Y%m%d" -d  "-1 days"`.txt

----------------------------------------------添加pv数据---------------------------
pvDataAll = load '$inputDirPv' using parquet.pig.ParquetLoader();
pvDataYHAll = filter pvDataAll by cid == '$selectCid'; 
pvData = foreach pvDataYHAll generate agsid;

-------------------------------pv join cm-----------------
pvcmDataSrc = load '$inputDirCM' using parquet.pig.ParquetLoader();
pvcmAllYH = filter pvcmDataSrc by channel == 'YangHe' and channelid is not null and channelid !='';--YH
pvcmAllYHTime = FOREACH pvcmAllYH GENERATE agsid,channelid,logtime;
pvcmAllYHGroup = Group pvcmAllYHTime by agsid;

--只留下最新agsid和channlid
pvcm = FOREACH pvcmAllYHGroup  {
    yhsorted = ORDER pvcmAllYHTime BY logtime DESC;
...
pvcmDataAll = join pvData by agsid, pvcm by agsid;
pvcmData = foreach pvcmDataAll GENERATE pvcm::channelid as YHID, pvcm::agsid as agsid;

------------------取签-------------------------
pvcmTagAll = join pvcmData by agsid LEFT OUTER, user by agsid;
pvcmTag = foreach pvcmTagAll GENERATE pvcmData::YHID as YHID, pvcmData::agsid as agsid, user::tag as tag;
-------------------pvcm join tt-------------------------------
pvYHTTDuplicateAgsid = join pvcmTag by agsid, dspttYH by agsid;
pvYHTT = foreach pvYHTTDuplicateAgsid GENERATE pvcmTag::YHID as YHID, pvcmTag::agsid as agsid, pvcmTag::tag as tag, dspttYH::mid as mid, dspttYH::channelContctgr as channelContctgr, dspttYH::conttitle as conttitle, dspttYH::ft as ft, dspttYH::province as province, dspttYH::city as city,dspttYH::channel as channel, dspttYH::ip as ip;

----------------------join ad3---------------------------------
pvYHTTAd3 = join pvYHTT by mid LEFT OUTER, ad3 by mid;
simplePvYHTTAd3 = foreach pvYHTTAd3 GENERATE pvYHTT::YHID as yangheid, pvYHTT::agsid as agsid, pvYHTT::tag as tag, pvYHTT::mid as mid, ad3::name as name, ad3::planid as planid, ad3::planname as planname, pvYHTT::channelContctgr as channelContctgr, pvYHTT::conttitle as conttitle, pvYHTT::ft as ft,pvYHTT::province as province, pvYHTT::city as city, pvYHTT::channel as channel, pvYHTT::ip as ip;
...
orderyanghedata = order yanghedata by yangheid;
STORE orderyanghedata INTO '$outputdirPv' USING PigStorage('\t');
